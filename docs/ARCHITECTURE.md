...
`lib/answer.ts` builds citations from the retrieved passages (`buildCitations`) with full `RAG.md` §8.1 provenance (provision + document version, jurisdiction, authority tier, temporal window, rendered `citation_text`, byte-exact quote). It computes the definitive `verification_status` via the deterministic structural rule `determineVerificationStatus` (RAG.md §8.3): all-CITED on TIER_1/TIER_2 ⇒ CITED; any weaker/UNVERIFIED grounding ⇒ PARTIAL; no passages ⇒ INSUFFICIENT_AUTHORITY. `requires_human_review` is set whenever the status is not CITED. When `LLM_API_KEY`+`LLM_MODEL` are configured it synthesizes a grounded answer; otherwise it returns a deterministic template answer citing the passages. The answer endpoint returns `verification_status` and `requires_human_review` (API.md §4.4).
