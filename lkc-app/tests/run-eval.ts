...
import { hybridRetrieve, vectorRetrieve, keywordRetrieve } from "../lib/retrieval";
...
export async function evaluateSingle(item: GoldItem, k: number) {
  ...
  // Branch diagnostics (Phase 5): would the target be found by vector-only or
  // keyword-only within the same top-k? Lets the eval attribute losses/recoveries.
  const [vectorOnly, keywordOnly] = await Promise.all([
    vectorRetrieve(item.question, { jurisdiction: item.jurisdiction, currentOnly: true, limit: k }),
    keywordRetrieve(item.question, { jurisdiction: item.jurisdiction, currentOnly: true, limit: k }),
  ]);
  const vectorHit = vectorOnly.some((p) => expected.has(p.provision_no)) ? 1 : 0;
  const keywordHit = keywordOnly.some((p) => expected.has(p.provision_no)) ? 1 : 0;
...
async function main() {
  const agg: Agg = { precisionSum: 0, recallSum: 0, citationHitSum: 0, count: 0, isolationFail: 0, vectorHit: 0, keywordHit: 0 };
  ...
  for (const item of gold) {
    ...
    agg.vectorHit += r.vectorHit;
    agg.keywordHit += r.keywordHit;
  }
  ...
  for (const r of rows) {
    ...
    console.log(
      `${String(r.id).padEnd(7)} P=${r.precision} R=${r.recall} cite=${r.citationHit} isoFail=${r.isolationFail} v=${r.vectorHit} k=${r.keywordHit}  exp=[${exp}] got=[${ret}]`
    );
  }
  ...
  console.log(`Branch diagnostics: vector-only hit ${agg.vectorHit}/${agg.count}, keyword-only hit ${agg.keywordHit}/${agg.count}`);
